...
const HybridTooltip = ({ label, maksud, formula, contoh, className = "" }: Props) => {
  ...
  const overlay = useMemo(() => {
      ...
      return createPortal(
        <div className="info-mobile-backdrop" onClick={() => setOpen(false)}>
          <div className="info-mobile-panel" role="dialog" aria-modal="true" aria-label={label} onClick={(event) => event.stopPropagation()}>
            <div className="info-overlay-header"><strong>{label}</strong><button type="button" className="info-overlay-close" onClick={() => setOpen(false)}>Tutup</button></div>
            <p><strong>Maksud:</strong> {maksud}</p><p><strong>Formula:</strong> {formula}</p><p><strong>Contoh:</strong> {contoh}</p>
          </div>
        </div>, document.body,
      );
    }
    ...
    return createPortal(
      <div className="info-overlay-content" role="tooltip" style={{ top, left }}>
        <p><strong>{label}</strong></p><p><strong>Maksud:</strong> {maksud}</p><p><strong>Formula:</strong> {formula}</p><p><strong>Contoh:</strong> {contoh}</p>
      </div>, document.body,
    );
